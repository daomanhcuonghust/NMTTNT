...
def parse_args():
    ...
    parser.add_argument('--fer_path', type=str, default='datasets/fer2013/images_fer2013/', help='fer2013 dataset path.')
...
class FerDataSet(data.Dataset):
    def __init__(self, fer_path, phase, transform = None):
        self.phase = phase
        self.transform = transform
        self.fer_path = fer_path

        df = pd.read_csv(os.path.join(self.fer_path, 'list_patition_label.txt'), sep=' ', header=None, names=['name','label'])
        ...
        for f in file_names:
            ...
            f = f.split("_")[1]
            f = f + '.jpg'
            path = os.path.join(self.fer_path, 'Image', f)
...
def run_training():
    ...
    train_dataset = FerDataSet(args.fer_path, phase = 'train', transform = data_transforms)    
    ...
    val_dataset = FerDataSet(args.fer_path, phase = 'test', transform = data_transforms_val)   
